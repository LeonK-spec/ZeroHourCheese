...
std::vector<Unity::il2cppFieldInfo*> Dumper::DumpFields(std::string component, std::string classname) {
    std::vector<Unity::il2cppFieldInfo*> methods_to_return;
    Unity::CGameObject* component_obj = Unity::GameObject::Find(component.c_str());
    if (!component_obj) {
        return methods_to_return;
    }
    ...
    ccc->FetchFields(&methods_to_return);

    return methods_to_return;
}

std::vector<std::string> Dumper::DumpFieldsString(std::string component, std::string classname) {
    std::vector<std::string> methods_to_return;
    printf("\nDumping fields of %s...\n", classname.c_str());

    std::vector<Unity::il2cppFieldInfo*> methods = Dumper::DumpFields(component, classname);

    for (Unity::il2cppFieldInfo* method : methods) {
        if (!method) {
            return methods_to_return;
        }
